...
| borrow_id | INT | FK → BorrowRecord, UNIQUE, NOT NULL | 关联借阅记录（一条借阅最多一条罚款，同一读者多次借阅可产生多条罚款） |
...
| role | VARCHAR(20) | NOT NULL | 该邀请码对应角色（系统管理员/图书管理员/读者） |
...
| Fine | 通过 borrow_id 关联借阅记录（UNIQUE约束保证一条借阅最多一条罚款），罚款金额由触发器根据规则计算生成，未冗余存储逾期天数或费率。同一读者的多次逾期借阅会产生多条独立的Fine记录 |
...
| fk_fine_borrow | Fine.borrow_id | BorrowRecord.borrow_id | 一对一（每条借阅最多产生一条罚款；读者多次借阅则对应多条Fine记录） |
